...
class RandomTweetWidget extends WP_Widget
{
    ...
    public function __construct()
    {
        ...
        parent::__construct(false, 'Random Tweet', array(), $control_ops);
    }
    ...
    protected function random_tweet_messages($options)
    {
        ...
        if (!class_exists('Codebird')) {
            ...
            require('lib/class-wp-codebird.php');
        }
        WP_Codebird::setConsumerKey($options['consumer_key'], $options['consumer_secret']);
        $this->cb = WP_Codebird::getInstance();
        ...
        if (empty($twitter_data) or count($twitter_data) < 1 or isset($twitter_data->errors)) {
            ...
            try {
                ...
                return __('Error retrieving tweets', 'random_tweet_widget');
            }
